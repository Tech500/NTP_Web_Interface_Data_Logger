////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//    Version 2.5       LittleFS version  09/22/2023 @ 00:25 EDT --WiFi Status updated
//
//                       ESP8266 --Internet Weather Datalogger and Dynamic Web Server   arduino.cc Topic
//
//                       "NTP_Web_Interface_Data_Logger.ino" developed by William Lucid
//
...
//
//                       https://observeredweather.000webhostapp.com/observations-I.html  Domain, Hosted web page
//
//
//  External            Note:  Compiled with ESP8266 Core Version 3.1.2 for "Arduino IDE."
//
///////////////////////////////////////////////////////////////////////////////////////////////////////
...
void setup(void)
{
  ...
  Serial.print("Version 2.5 'NTP_Web_Interface_Data_Logger.ino' 09/22/2023 @ 00:25 EDT");
  Serial.print("\n");

  wifi_Start();

  secondTick.attach(1, ISRwatchdog);  //watchdog  ISR "watchdogCounter" increments every 1 second
...
void links()
{
  ...
  client.println("<br>");
  client.println("<a href=http://" + publicIP + ":" + LISTEN_PORT + "/LOG.TXT >Current Week Observations</a><br>");
  client.println("<br>");
  ...
  client.println("<br>");
  client.println("<a href=http://" + publicIP + ":" + LISTEN_PORT + "/README.TXT >Server:  README</a><br>");
  client.println("<br>");
  client.println("<a href=https://www.hackster.io/storms/internet-weather-datalogger-and-dynamic-web-server-7860de>Project Discussion</a><br>");
  client.println("<br>");
    ...
    if (parsed)
    {
      ...
      if (strcmp(action, "GET") == 0)
      {
        ...
        else if ((strncmp(path, "/LOG", 4) == 0) ||  (strcmp(path, "/WIFI.TXT") == 0) || (strcmp(path, "/DIFFER.TXT") == 0) || (strcmp(path, "/SERVER.TXT") == 0) || (strcmp(path, "/README.TXT") == 0))   // Respond with the path that was accessed.
        {
          ...
          if ((strncmp(path, "/DIFF", 9) == 0) || (strncmp(path, "/ACCESS", 7) == 0))
          {
        ...
        else if (strncmp(path, Restricted, 14) == 0)  //replace "/zzzzzzz" with your choice.  Makes "ACCESS.TXT" a restricted file.  Use this for private access to remote IP file.
        {
          ...
          const char *filename = "/ACCESS.TXT";
          strcpy(MyBuffer, filename);

          // send a standard http response header
          client.println("HTTP/1.1 200 OK");
          client.println("Content-Type: text/plain");
          client.println("Content-Disposition: inline");  //Display file
          //client.println("Content-Disposition: attachment");  //Download file
          //client.println("Content-Length:");
          client.println();
        ...
        else
        {
          ...
          client.println("<h2>File Not Found!</h2>");
          client.println("<br><br>");
          client.println("<a href=http://" + publicIP + ":" + LISTEN_PORT + "/Weather   >Return to Current Observations</a><br>");
...
void readFile()
{
  ...
  File webFile = LittleFS.open(filename, "r");
